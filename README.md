...
| Oryza sativa v7.0  | Phytozome  | https://phytozome-next.jgi.doe.gov/  |
...
# Oryza
fasta = open('osa1_r7.gene_models.pep.fa','r').read().split('>')[1:]
out = open('osa1_r7.gene_models.pep.rename.fa','w')
# record all sequences - take the longest protein per gene
seq_d = {}
for seq in fasta:
    gene = seq.split(' ')[0].split('.')[0]
    sequence = seq.split('\n',1)[1].replace('\n','').replace('*','')
    try:
        if len(seq_d[gene]) < len(sequence):
            seq_d[gene] = sequence
...
for fname in glob('*rename.fa'):
    ...
    # get the 100 best hits per gene
    ...
    for h in blast:
        if '.' in h.split('\t')[1]: # exclude pdb
            ...
            try:
                if len(gene_to_hit[h.split('\t')[0]]) == 100:
                    pass
                else:
...
```
for i in *taxonomy; do awk '$2 == "Prokaryota" && $3 >= 50 && $4 == 100' $i > $i.filt; done
